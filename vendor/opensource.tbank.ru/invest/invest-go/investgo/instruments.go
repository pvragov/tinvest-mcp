...
func (is *InstrumentsServiceClient) StructuredNoteBy(ir *pb.InstrumentRequest) (*StructuredNoteByResponse, error) {
	var header, trailer metadata.MD
	resp, err := is.pbClient.StructuredNoteBy(is.ctx, ir, grpc.Header(&header), grpc.Trailer(&trailer))
	if err != nil {
		header = trailer
	}
	return &StructuredNoteByResponse{
		StructuredNoteResponse: resp,
		Header:                 header,
	}, err
}

// StructuredNotes — список структурных нот
func (is *InstrumentsServiceClient) StructuredNotes(ir *pb.InstrumentsRequest) (*StructuredNotesResponse, error) {
	var header, trailer metadata.MD
	resp, err := is.pbClient.StructuredNotes(is.ctx, ir, grpc.Header(&header), grpc.Trailer(&trailer))
	if err != nil {
		header = trailer
	}
	return &StructuredNotesResponse{
		StructuredNotesResponse: resp,
		Header:                  header,
	}, err
}

// DfaBy — получить цифровой актив по ее идентификатору
func (is *InstrumentsServiceClient) DfaBy(ir *pb.InstrumentRequest) (*DfaResponse, error) {
	var header, trailer metadata.MD
	resp, err := is.pbClient.DfaBy(is.ctx, ir, grpc.Header(&header), grpc.Trailer(&trailer))
	if err != nil {
		header = trailer
	}
	return &DfaResponse{
...
// StructuredNotes — список структурных нот
func (is *InstrumentsServiceClient) Dfas(r *pb.DfasRequest) (*DfasResponse, error) {
	var header, trailer metadata.MD
	resp, err := is.pbClient.Dfas(is.ctx, r, grpc.Header(&header), grpc.Trailer(&trailer))
	if err != nil {
		header = trailer
	}
	return &DfasResponse{
