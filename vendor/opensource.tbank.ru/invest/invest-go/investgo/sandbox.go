...
func (s *SandboxServiceClient) GetSandboxOrderPrice(accountID, instrumentID string, price *pb.Quotation,
	direction pb.OrderDirection, quantity int64) (*GetOrderPriceResponse, error) {
	var header, trailer metadata.MD
	resp, err := s.pbClient.GetSandboxOrderPrice(s.ctx, &pb.GetOrderPriceRequest{
		AccountId:    accountID,
		InstrumentId: instrumentID,
		Price:        price,
		Direction:    direction,
...
// PostSandboxStopOrder - Метод выставления стоп-заявки
func (s *SandboxServiceClient) PostSandboxStopOrder(req *PostStopOrderRequest) (*PostStopOrderResponse, error) {
	var header, trailer metadata.MD
	resp, err := s.pbClient.PostSandboxStopOrder(s.ctx, &pb.PostStopOrderRequest{
		Quantity:           req.Quantity,
		Price:              req.Price,
...
// GetSandboxStopOrders - Метод получения списка активных стоп заявок по счёту
func (s *SandboxServiceClient) GetSandboxStopOrders(accountId string) (*GetStopOrdersResponse, error) {
	var header, trailer metadata.MD
	resp, err := s.pbClient.GetSandboxStopOrders(s.ctx, &pb.GetStopOrdersRequest{
		AccountId: accountId,
	}, grpc.Header(&header), grpc.Trailer(&trailer))
	if err != nil {
		header = trailer
...
func (s *SandboxServiceClient) CancelStopOrder(accountId, stopOrderId string) (*CancelStopOrderResponse, error) {
	var header, trailer metadata.MD
	resp, err := s.pbClient.CancelSandboxStopOrder(s.ctx, &pb.CancelStopOrderRequest{
		AccountId:   accountId,
